...
class VirtualPos{
    ...
    public function crearSuscripcion(string $emailCliente, 
        ...
        $request=["automatic_renewal"=> "F",
                ...
                "return_url"=>base64_encode($url_return),
                "callback_url"=>base64_encode($url_return."_async")];
    ...
    public function cancelarCargo(string $uuid){
        $signature = $this->login();
        $client = new Client();
        try{   
            
            $response = $client->request( "delete", $this->url."/charge/".$uuid,[
            ...
            if($response->getStatusCode()==200){
                ...
                if($responseBody["code"]==200){
                    ...
                    throw new Exception("Ha ocurrido un error al cancelar la cuota - codigoError: ".$response->getStatusCode());
            }
        }catch(\Exception $e){
            throw new Exception($e->getMessage());
        }         
