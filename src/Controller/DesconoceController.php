...
class DesconoceController  extends AbstractController{
    ...
    public function index(ContratoRepository $contratoRepository,PaginatorInterface $paginator,ModuloPerRepository $moduloPerRepository,Request $request,CuentaRepository $cuentaRepository): Response
    {
        ...
        $contratos=$paginator->paginate(
            ...
            20 /*limit per page*/,
            array('defaultSortFieldName' => 'fechaDesiste', 'defaultSortDirection' => 'Asc'));
               ...
               // $toku=new Toku();
                ...
                if($contrato->getEstadoSuscripcion()=="ACTIVA"){
                    ...
                    try{
                        ...
                        $response = $virtualPos->cancelarSuscripcion($contrato->getSuscripcionId());
                    ...
                    if($cuota->getIsMulta()==null){
                        ...
                        if($contrato->getEstadoSuscripcion()=="ACTIVA"){
                            ...
                            try{
                                ...
                                $virtualPosLog->setFechaRegistro(new \DateTime(date("Y-m-d")));
                                $virtualPosLog->setResponse($ex->getMessage());
                                $virtualPosLog->setRequest(json_encode($cuota));
                                $entityManager->persist($virtualPosLog);
                                $entityManager->flush();
                            }
                        }
