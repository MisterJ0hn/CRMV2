...
class ContratoConsultorController extends AbstractController
{
        ...
        try {
            ...
            if ($isMoroso) {
                ...
                $tabCondicion = "EXISTS (
                    ...
                    AND (cu2.pagado IS NULL OR cu2.monto > cu2.pagado+".$configuracion->getDeudaMinima().")
            ...
            if($e->getCode()==403){
                ...
                return new Response("<div class='alert bg-danger' >No fue posible acceder a los datos. Su sesión puede haber expirado. Por favor, vuelva a iniciar sesión.</div>");
            }else{
                return new Response("<div class='alert bg-danger' >".$e->getCode()." - ".$e->getMessage()."</div>");
    ...
    /**
     * @Route("/tab_export", name="contrato_consultor_tab_export", methods={"GET"})
     */
    public function tabExport(ContratoRepository $contratoRepository,
                            PaginatorInterface $paginator,
                            ConfiguracionRepository $configuracionRepository,
                             
                            Request $request): Response
    {
        $this->denyAccessUnlessGranted('view','contrato_consultor_exportar');
        $user = $this->getUser();
       ...
       $items= $query->getResult();
        ...
        foreach ($items as $contrato) {
            ...
            $sheet->setCellValue("A$i",$contrato->getFolio());
            $sheet->setCellValue("B$i",$contrato->getGrupo()->getId());
        ...
        $fileName = $tab.'_'.date("YmdHi").'.xlsx';
