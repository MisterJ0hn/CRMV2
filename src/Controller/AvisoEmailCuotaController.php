...
/**
 * @Route("aviso_email_cuota")
 */
class AvisoEmailCuotaController extends AbstractController
{
    /**
     * @Route("/", name="aviso_email_cuota_index")
     */
    public function index(ModuloPerRepository $moduloPerRepository): Response
    {
        $this->denyAccessUnlessGranted('view','aviso_email_cuota');
        $user=$this->getUser();
        $pagina=$moduloPerRepository->findOneByName('aviso_email_cuota',$user->getEmpresaActual());
        

        $user=$this->getUser();
        return $this->render('aviso_email_cuota/index.html.twig', [
            'controller_name' => 'AvisoEmailCuotaController',
            'pagina'=>$pagina->getNombre(),
        ]);
    }

    /**
     * @Route("/excel", name="aviso_email_cuota_excel", methods={"GET"})
     */
...
/*
        ...
        foreach($query as $cuota){
           ...
           // $otros=' DATEDIFF(\''. $dateInicio.'\',c.fechaPago)>='.$vencimiento[0]->getValMax();
                ...
                $sheet->setCellValue("C$i", $cuota->getContrato()->getEmail());
                $sheet->setCellValue("D$i", $cuota->getContrato()->getTelefono());
                
                $sheet->setCellValue("E$i", $cuota->getMonto());
                $sheet->setCellValue("F$i",  $cuota->getFechaPago()->format('d'). " de ".$contratoFunciones->mesToEspanol($cuota->getFechaPago()->format('F'))." de ".$cuota->getFechaPago()->format('Y'));
              $i++;
        ...
        $fileName = 'AvisoMailCuota_'.date('dmy_Hs').'.xlsx';
