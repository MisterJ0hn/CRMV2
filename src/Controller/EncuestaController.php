...
class EncuestaController extends AbstractController
{
    ...
    /**
     * @Route("/obtenerContenido", name="encuesta_obtener_contenido", methods={"GET"})
     */
    ...
    public function resumenencuestadores(Request $request, VwContratoRepository $vwContratoRepository): JsonResponse
    {
        ...
        $fechainicio = $request->query->get('fechainicio', date('Y-m-d'));
        $fechafin    = $request->query->get('fechafin', date('Y-m-d'));
        $status      = $request->query->get('status');
        $tipoFecha   = $request->query->get('tipoFecha', 0);
