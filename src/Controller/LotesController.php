...
class LotesController extends AbstractController
{
    ...
    /**
     * @Route("/{id}/por_equipos", name="lotes_por_equipos", methods={"POST","GET"})
     */
    public function porEquipos(EquipoTrabajo $equipoTrabajo, LotesRepository $lotesRepository):JsonResponse
    {
        $lotes = $lotesRepository->findHabilitados($equipoTrabajo->getId());
        $lotes_array = array_map(function(Lotes $lot)
        {
            return ['id'=>$lot->getId(),'nombre'=>$lot->getNombre()];
        },$lotes);
        return $this->json($lotes_array,200);
    }
    /**
