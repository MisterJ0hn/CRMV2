...
class ApiController extends AbstractController
{
                    ...
                    ): JsonResponse 
    {
        ...
        $apiToken->setExpiresAt(new \DateTime('+30 minute'));
                            ...
                            ConfiguracionRepository $configuracionRepository): JsonResponse
    {
        ...
        try {
            ...
            $apiLlamado->setExito(0);
            $apiLlamado->setMensajeError($e->getLine()." - ".$mensajeErroApi);
            $em->persist($apiLlamado);
    ...
    */
    ...
    function registrarDocumento(Request $request, 
                            ...
                            PjudMovimientoRepository $pjudMovimientoRepository
                            ){
        $apiLlamado = new ApiLlamado();
        $data = json_decode($request->getContent(), true);
        ...
        try{
            ...
            $causaEbook = $pjudCausaRepository->findOneBy(['docEbook'=>$nombreArchivo,'id'=>$causa->getId()]);
            if($causaEbook){
                $causaEbook->setDocEbookDescargado(1);
                $em->persist($causaEbook);
                $em->flush();
            }
            $causaDemandaDescargado = $pjudCausaRepository->findOneBy(['docDemanda'=>$nombreArchivo,'id'=>$causa->getId()]);
            if($causaDemandaDescargado){
                $causaDemandaDescargado->setDocDemandaDescargado(1);
                $em->persist($causaDemandaDescargado);
                $em->flush();
            }
            $causaCertificadoEnvioDescargado = $pjudCausaRepository->findOneBy(['docCertificadoEnvio'=>$nombreArchivo,'id'=>$causa->getId()]);
            ...
            $anexoCausaDoc = $pjudAnexoCausaRepository->findOneBy(['doc'=>$nombreArchivo,'pjudCausa'=>$causa->getId()]);
             if($anexoCausaDoc){
            ...
            $movimientos= $pjudMovimientoRepository->findBy(['pjudCausa'=>$causa->getId()]);
            foreach($movimientos as $mov){
                $anexoMovimientoDoc = $pjudAnexoMovimientoRepository->findOneBy(['nombreArchivo'=>$nombreArchivo,'pjudMovimiento'=>$mov->getId()]);
                if($anexoMovimientoDoc){
                    $anexoMovimientoDoc->setArchivoDescargado(1);
                    $em->persist($anexoMovimientoDoc);
                    $em->flush();
                }
                $pdfMovimiento = $pjudPdfRepository->findOneBy(['nombreArchivo'=>$nombreArchivo,'pjudMovimiento'=>$mov->getId()]);
                if($pdfMovimiento){
            ...
            $escritos = $pjudEscritosRepository->findOneBy(['doc'=>$nombreArchivo,'pjudCausa'=>$causa->getId()]);
            if($escritos){
    ...
    /**
     * @Route("/api/adereso/consultar", methods={"GET"})
     */
    public function adeResoConsultar(
        Request $request,
        EntityManagerInterface $em,
        ContratoRepository $contratoRepository
    ): JsonResponse {
        $log = new ApiLlamadoAdereso();
        ...
        if ($telefono === '') {
            $log->setExito(false);
            $log->setMensajeError('Parámetro telefono requerido');
            $em->persist($log);
            $em->flush();
            return $this->json(['error' => 'Parámetro telefono requerido'], 400);
        }

        $log->setTelefono($telefono);

        try {
            $contratos = $contratoRepository->findByTelefono($telefono);

            if (empty($contratos)) {
                $log->setExito(false);
                $log->setMensajeError('No se encontraron contratos para el teléfono: ' . $telefono);
                $em->persist($log);
                $em->flush();
                return $this->json(['error' => 'Cliente no encontrado'], 404);
            }

            /** @var Contrato $primero */
            $primero = $contratos[0];

            $contratosData = array_map(function (Contrato $c) {
                $materia = null;
                if ($c->getCartera() && $c->getCartera()->getMateria()) {
                    $materia = $c->getCartera()->getMateria()->getNombre();
                } elseif ($c->getMateriaOrigen()) {
                    $materia = $c->getMateriaOrigen();
                }
                return [
                    'folio'   => $c->getFolio(),
                    'materia' => $materia,
            ...
            $log->setExito(false);
            $log->setMensajeError($e->getLine() . ' - ' . $e->getMessage());
            $em->persist($log);
            $em->flush();
