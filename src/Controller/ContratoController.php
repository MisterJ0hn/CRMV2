...
class ContratoController extends AbstractController
{
            ...
            if (!empty($items)) {
                ...
                try {
                    ...
                    $sql = "
                        SELECT
                            c.id,
                            COALESCE(TO_DAYS(NOW()) - TO_DAYS(c.fecha_ultima_observacion_general), 0) AS dias_ult_observacion,
                    ...
                    foreach ($items as $contrato) {
                        ...
                        if ($row) {
                            if($contrato->getFechaUltimaObservacionGeneral()!=null){
                    ...
                    $errorSistema->setModulo("contrato_obtener_contenido");
                    ...
                    \Knp\Snappy\Pdf $snappy): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $contrato->setFechaUltimaModificacion(new \DateTime(date("Y-m-d H:i:s")));
            ...
            $materiaAcual=0;
            foreach($agenda->getCuenta()->getCuentaMaterias() as $cuentaMateria){
                $materiaAcual=$cuentaMateria->getMateria()->getId();
            ...
            if($contrato->getAceptaSuscripcion()==null){
                if(!$tienePago && count($contrato->getContratoAnexos())==0){
                    ...
                    foreach($detalleCuotas as $detalleCuota){
                        ...
                        $detalleCuota->setFechaAnulacion(new \DateTime(date("Y-m-d H:i:s")));
                        $detalleCuota->setUsuarioAnulacion($user);                     
                    ...
                    if($isTotal==true){
                        $contrato->setFechaPrimeraCuota(new \DateTime($request->request->get('txtFechaTotal')));
                        $cuota=new Cuota();
                        $cuota->setContrato($contrato);
                        $cuota->setNumero($numeroCuota);
                        $cuota->setFechaPago($contrato->getFechaPrimeraCuota());
    ...
    public function pdf(Contrato $contrato,ContratoMeeRepository $contratoMeeRepository, ContratoRepository $contratoRepository): Response
    {
        ...
        try{
            ...
            if($contrato->getAceptaSuscripcion()){
                ...
                $this->addFlash('success',"Contrato creado con exito!!");
                return $this->redirectToRoute('contrato_index');
            }
        }catch(\Exception $e){
            try {
                ...
                $errorSistema->setMensaje($e->getMessage());
                $errorSistema->setFecha(new \DateTime(date("Y-m-d H:i:s")));
                $errorSistema->setModulo("contrato_pdf");
                $em->persist($errorSistema);
                $em->flush();
            } catch(\Exception $logException) {}

            $this->addFlash('error','No fue posible crear el pdf.');
            return $this->redirectToRoute('contrato_index');
                    ...
                    ContratoFunciones $contratoFunciones): Response
    {
        ...
        if(null !== $request->query->get('status')){
            ...
            try{
                ...
                $virtualPosLog->setContrato($contrato);
                $virtualPosLog->setResponse($e->getMessage());
                $virtualPosLog->setRequest("contrato_pdf");
                $virtualPosLog->setFechaRegistro(new \DateTime(date("Y-m-d H:i:s")));
                $entityManager->persist($virtualPosLog);
                $entityManager->flush();
            }
                        ...
                        UsuarioGrupoRepository $usuarioGrupoRepository): Response
    {
        ...
        if($cuenta_materia->getMateria()->getId()!=1){
                $usuarioConsultor = $contrato->getTramitador()->getNombre();
        }else{
            $usuarioGrupo = $usuarioGrupoRepository->findOneBy(['grupo'=>$contrato->getGrupo()->getId()]);
                $usuarioConsultor = $usuarioGrupo->getUsuario()->getNombre();
        }
        ...
        return $this->render('contrato/lineaTiempo.html.twig', [
	        ...
	        'consultor'=>$usuarioConsultor,
            'materiaId'=>$cuenta_materia->getMateria()->getId(),
        ]);
    }
    /**
                                    ...
                                    ): JsonResponse
    {
        ...
        try{
            if($request->request->get('observacion')){
                ...
                $causaObservacion = new CausaObservacion();
                $contrato=$detalleCuaderno->getCausa()->getAgenda()->getContrato();
                

                $causaObservacion->setCausa($detalleCuaderno->getCausa());
                $causaObservacion->setUsuarioRegistro($user);
                $causaObservacion->setContrato($contrato);
                $causaObservacion->setFechaRegistro(new \DateTime(date("Y-m-d H:i")));
                $causaObservacion->setObservacion($observacion);
                $causaObservacion->setDetalleCuaderno($detalleCuaderno);
                ...
                $contrato->setFechaUltimaObservacionGeneral(new DateTime(date('Y-m-d H:i:s')));
                $em->persist($contrato);
                $em->flush();
    ...
    function lineaTiempoDetalle(Causa $causa, Request $request,
                                ...
                                CuadernoRepository $cuadernoRepository,
                                DetalleCuadernoRepository $detalleCuadernoRepository,
                                MensajeTipoRepository $mensajeTipoRepository
                                ): Response
    {
        $this->denyAccessUnlessGranted('view','linea_tiempo');
        $user=$this->getUser();
        ...
        $form->add('fechaAviso',DateTimeType::class,array('widget'=>'single_text','html5'=>false));
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $entityManager = $this->getDoctrine()->getManager();
            $mensaje->setObservacion($causa->getLetra()."-".$causa->getRol()."-".$causa->getAnio()." - ". $mensaje->getObservacion());
            $mensaje->setCausa($causa);
            $entityManager->persist($mensaje);
            $entityManager->flush();

            
            $origen = $request->getSession()->get('origen');
            if($origen != null){
                if($origen == 'Mis_Clientes'){
                    return $this->redirectToRoute('mis_clientes_index');
                }
            }
                                    ...
                                    Request $request): Response
    {
        ...
        foreach ($etapas as $etapa) {
            ...
            $etapa_pendiente=$lineaTiempoEtapaRepository->obtenerEtapaPendiente($causa->getId(),$causa->getMateriaEstrategia()->getEstrategiaJuridica()->getLineaTiempo()->getId());
            if($etapa_pendiente!=null){
                $causa->setEtapaPendiente($etapa_pendiente->getNombre());
            }else{
                $causa->setEtapaPendiente('');
            }
            $em->persist($causa);
            $em->flush();
        }
                        ...
                        Request $request): Response
    {
        ...
        if($request->request->get('txtObservacion')!=null){
            ...
            try{
                ...
                $errorSistema->setMensaje($e->getMessage());
                $errorSistema->setFecha(new \DateTime(date("Y-m-d H:i:s")));
                $errorSistema->setModulo("contrato_observacion_sac");
                $em->persist($errorSistema);
                $em->flush();
            }
                            ...
                            Request $request): Response
    {
        ...
        try{
            ...
            file_put_contents(
                '/home/ejam.cl/crm/public_html/error_sistema.log',
    ...
    {
    	$this->denyAccessUnlessGranted('view','reasignar_cartera');
        $user = $this->getUser();
        ...
        if($request->request->get('cboCarteras')){
            ...
            //buscamos los tickets para reasignar
            
            $tickets = $ticketRepository->findBy(['contrato'=>$contrato,'estado'=>2, 'destino'=>7]);
            foreach ($tickets as $ticket) {
             //   $ticket->setCartera($cartera);
                if($ticket->getEncargado()->getId() != $contrato->getTramitador()->getId()){
                    
                    $encargado_anterior=$ticket->getEncargado();
                    $ticket->setEncargado($contrato->getTramitador());
                
                    $entityManager->persist($ticket);
                    $entityManager->flush();

                    $ticketHistorico = new TicketHistorial();
                    $ticketHistorico->setTicket($ticket);
                    $ticketHistorico->setFecha(new DateTime(date('Y-m-d H:i:s')));
                    $ticketHistorico->setEstado($ticket->getEstado());
                    $ticketHistorico->setUsuarioRegistro($contrato->getTramitador());
                    $ticketHistorico->setObservacion("Reasignado desde cartera contrato:
                                                    <br> <br>
                                                    <div class='row'><div class='col-12 col-md-12 col-sm-12 col-lg-12 col-xl-6'><strong>ReasigandoPor</strong>:</div><div class='col-12 col-md-12 col-sm-12 col-lg-12 col-xl-6'> ".$user->getNombre()."</div></div>".
                                                    "<div class='row'><div class='col-12 col-md-12 col-sm-12 col-lg-12 col-xl-6'><strong>Cartera</strong>:</div><div class='col-12 col-md-12 col-sm-12 col-lg-12 col-xl-6'>".$cartera->getNombre()."</div></div>".
                                                    "<div class='row'><div class='col-12 col-md-12 col-sm-12 col-lg-12 col-xl-6'><strong>TramitadorAnterior</strong>:</div><div class='col-12 col-md-12 col-sm-12 col-lg-12 col-xl-6'>".$encargado_anterior->getNombre()."</div></div>");
            
                    $entityManager->persist($ticketHistorico);
                    $entityManager->flush();
                }
            }
