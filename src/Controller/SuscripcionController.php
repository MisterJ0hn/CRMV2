...
class SuscripcionController extends AbstractController
{
     ...
     */
    public function registrarPagoFallido(Request $request,
                                CuotaRepository $cuotaRepository
                                ): JsonResponse
    {
        ...
        try{
            ...
            $historicoSuscripcion->setFechaRegistro(new \DateTime(date("Y-m-d H:i:s")));
            $historicoSuscripcion->setExito(true);
            $historicoSuscripcion->setObservacion("<p>Pago Rechazado </p><p>detalle de cuota:</p>
                                                    <ul>
                                                        <li>número cuota:".$cuota->getNumero()."</li>
                                                        <li>Monto Pago: $".$cuota->getMonto()."</li>
                                                        <li>Fecha Pago: ".$cuota->getFechaPago()->format("Y-m-d")."</li>
                                                    </ul>");
        ...
        if(null !== $contrato and $contrato->getSesionSuscripcionActiva()==1){
            ...
            if($form->isSubmitted() && $form->isValid()){
                ...
                try{
                    ...
                    $error='<div class="card bg-danger p-0">
                            
                                    <h3 class="card-title">Suscripción no realizada</h3>
             ...
             $error='<div class="card bg-danger p-0">
                                    ...
                                    <h3 class="card-title">La sesión no es válida</h3>                         </div>';
        }
    ...
    {
        $contrato = $contratoRepository->findOneBy(["sesionSuscripcion"=>$uuid]);
        if($contrato->getSesionSuscripcionActiva()==1 && $contrato->getSuscripcionValidacion()==null){
            $entityManager = $this->getDoctrine()->getManager();
            $contrato->setSuscripcionValidacion(true);
            $entityManager->persist($contrato);
            $entityManager->flush();
            $error="aceptado";
            $entityManager = $this->getDoctrine()->getManager();
            ...
            try{
                ...
                if($response["suscription"]["status"]=="ACTIVA"){
                    $cuotas = $cuotaRepository->findBy(["contrato"=>$contrato, "anular"=>null,"pagado"=>[null,0]],["fechaPago"=>"ASC"]);
                    ...
                    $contrato->setSuscripcionValidacion(null);
                    $entityManager->persist($historicoSuscripcion);
                ...
                $historicoSuscripcion->setFechaRegistro(new \DateTime(date("Y-m-d H:i:s")));
                $historicoSuscripcion->setExito(false);
                $historicoSuscripcion->setObservacion("ha ocurrido un error al suscribir, intentelo nuevamente. <br>Detalle error: ".$e->getMessage());
                $historicoSuscripcion->setSuscripcionId($contrato->getSuscripcionId());
                $entityManager->persist($historicoSuscripcion);
                $entityManager->flush();
                       ...
                       ):JsonResponse
    {
        ...
        if($contrato->getSesionSuscripcionActiva()==1 && $contrato->getSuscripcionValidacionCallback()==null){
            $entityManager = $this->getDoctrine()->getManager();
            $contrato->setSuscripcionValidacionCallback(1);
            ...
            if($response["suscription"]["status"]=="ACTIVA"){
                try{
                    ...
                    $contrato->setContratoEstadoSuscripcion($contratoEstadoSuscripcionRepository->find(3));
                    $contrato->setEstadoSuscripcion("Error");
                    $contrato->setSuscripcionValidacionCallback(null);
                    $entityManager->persist($contrato);
                    $entityManager->flush();
    ...
    public function inscribir(Contrato $contrato,Configuracion $configuracion,ContratoEstadoSuscripcion $contratoEstadoSuscripcion,$cuotas,$url_return){
        ...
        $response = $virtualPos->crearSuscripcion($contrato->getCliente()->getCorreo(),
                                                ...
                                                $url_return,
                                                $contrato->getId());
        if($response["url_redirect"]== null){
            throw new \Exception("<p>Posibles causas:</p> <ul><li>Correo mal tipeado</li><li>Teléfono incorrecto</li><li>Rut mal formateado</li></ul>");
        }
