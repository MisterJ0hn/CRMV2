...
class PanelAbogadoController extends AbstractController
{
    ...
    public function contrata(Agenda $agenda,Request $request,
                            ...
                            LineaTiempoEtapasRepository $lineaTiempoEtapasRepository
                            ):Response
    {
        $this->denyAccessUnlessGranted('create','panel_abogado');
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            try{
                ...
                $contrato->setCompaniaOrigen($contrato->getAgenda()->getCuenta()->getNombre());
                $materia_id=0;
                foreach ($materias as $materia) {
                    $materia_id=$materia->getMateria()->getId();
                    $contrato->setMateriaNombre($materia->getMateria()->getNombre());
                ...
                if($cuotasGrabadasPreviamente==null){
                    ...
                    if($contrato->getIsAbono()==true || $contrato->getIsTotal()==true){
                        ...
                        $dias_morosidad = $contrato->getFechaPrimeraCuota()->diff(new \DateTime())->days;
                ...
                if($contrato->getAgenda()->getIdGhl()!=""){
                    $tramitacion = new Tramitacion();
                    $materiaNombre="";
                    foreach($cuenta_materia as $cm){
                ...
                foreach ($causas as $causa) {
                    try{
                       ...
                       /* $pjudScraping->enviarDatos($token,
                                                    ...
                                                    $causa->getMateriaEstrategia()->getMateria()->getPjudCompetenciaId()??"0",
                ...
                $errorSistema->setModulo("panel_abogado_contrata");
                    ...
                    ): Response
    {
        ...
        if(null !==$request->request->get('hdNoContrata')){
           ...
           // $agenda->setObservacion($agenda->getObservacion()."<hr>".$request->request->get('txtObservacion'));
            ...
            $observacion=new AgendaObservacion();
            $observacion->setAgenda($agenda);
            $observacion->setSubStatus($agendaSubStatusRepository->find($request->request->get('cboSubStatus')));
            $observacion->setUsuarioRegistro($usuarioRepository->find($user->getId()));
     ...
     */
    public function postergado(Agenda $agenda,Request $request,
                    AgendaStatusRepository  $agendaStatusRepository,
                    AgendaSubStatusRepository $agendaSubStatusRepository,
                    UsuarioRepository $usuarioRepository
                    ): Response
    {
        ...
        return $this->render('panel_abogado/postergado.html.twig', [
            'agenda'=>$agenda,
            'status'=>$_GET['status']
        ]);
    }
