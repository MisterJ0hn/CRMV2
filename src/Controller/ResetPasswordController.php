...
class ResetPasswordController extends AbstractController
{
    ...
    public function reset(Request $request, UserPasswordEncoderInterface $passwordEncoder, PasswordService $passwordService, string $token = null): Response
    {
        ...
        if (null === $token) {
            throw $this->createNotFoundException('No se encontró ningún token de restablecimiento de contraseña en la URL ni en la sesión.');
        }
    ...
    private function processSendingPasswordResetEmail(string $usernameFormData, MailerInterface $mailer): RedirectResponse
    {
        ...
        try {
            ...
            $this->addFlash('reset_password_error', sprintf(
                'Se produjo un problema al procesar su solicitud de restablecimiento de contraseña - %s',
                $e->getReason()
            ));
        ...
        $email = (new TemplatedEmail())
            ->from(new Address('noreply@ejam.cl', 'Administrador'))
            ->to($user->getCorreo())
