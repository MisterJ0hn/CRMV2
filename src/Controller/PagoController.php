...
class PagoController extends AbstractController
{
    ...
    public function index( VwCuotaConEquipoRepository $cuotaRepository,PaginatorInterface $paginator,ModuloPerRepository $moduloPerRepository,Request $request,CuentaRepository $cuentaRepository): Response
    {
        ...
        switch($user->getUsuarioTipo()->getId()){
            ...
            case 8:
            case 12:
                $query=$cuotaRepository->findVencimiento(null,null,null,$filtro,null,true,$fecha,true,null,null,$rut);
                                $companias=$cuentaRepository->findByPers(null,$user->getEmpresaActual());
                break;
            case 7://tramitador
                $query=$cuotaRepository->findVencimiento($user->getId(),null,null,$filtro,7,true,$fecha,true,null,null,$rut);
                $companias=$cuentaRepository->findByPers(null,$user->getEmpresaActual());
                break;
            case 6: //abogado
                $query=$cuotaRepository->findVencimiento($user->getId(),null,null,$filtro,6,true,$fecha,true,null,null,$rut);
                $companias=$cuentaRepository->findByPers(null,$user->getEmpresaActual());
                break;
                                    ...
                                    CuentaRepository $cuentaRepository): Response
    {
        ...
        return $this->render('pago/index_primera_cuota.html.twig', [
            ...
            'pagina'=>$pagina->getNombre(),
            'finalizado'=>false,
            'TipoFiltro'=>'PagoPrimeraCuota'
        ]);
    }
    /**
     * @Route("/primera_cuota_ver_pagos/{id}", name="primera_cuota_contrato_show", methods={"GET"})
     */
    public function primeraCuotaVerPagosShow(Contrato $contrato,CuotaRepository $cuotaRepository,ModuloPerRepository $moduloPerRepository): Response
    {
        $this->denyAccessUnlessGranted('view','pago_index_primera_cuota');
        $user=$this->getUser();
        $pagina=$moduloPerRepository->findOneByName('pago',$user->getEmpresaActual());
        ...
        return $this->render('pago/verpagos_show.html.twig', [
            ...
            'contrato'=>$contrato,
            'cuotas' => $cuotaRepository->findBy(['contrato'=>$contrato->getId()],["id"=>'asc'])
        ]);
                        ...
                        VencimientoRepository $vencimientoRepository): Response
    {
        ...
        $otros='';
        $tipo_fecha=0;
        $bUsuarioRegistro=null;
        if(null !== $request->query->get('bUsuarioRegistro') && $request->query->get('bUsuarioRegistro')!=0){
            $bUsuarioRegistro=$request->query->get('bUsuarioRegistro');
        }
        ...
        switch($user->getUsuarioTipo()->getId()){
            ...
            case 8:
            case 12:
                $query=$pagoRepository->findByPers(null,null,$compania,$filtro,$fecha,$bUsuarioRegistro);
                break;
            case 11://Administrativo
                $query=$pagoRepository->findByPers($user->getId(),null,$compania,$filtro,$fecha,$bUsuarioRegistro);
    ...
    */
    ...
    public function finalizado(ContratoRepository $contratoRepository, VwCuotaConEquipoRepository $cuotaRepository,PagoRepository $pagoRepository,PaginatorInterface $paginator,ModuloPerRepository $moduloPerRepository,Request $request,CuentaRepository $cuentaRepository): Response
    {
        ...
        switch($user->getUsuarioTipo()->getId()){
            ...
            case 8:
            case 12:
                $query=$cuotaRepository->findVencimiento(null,null,null,$filtro,7,false,$fecha,$conrestriccion,false,null,$rut);
                $companias=$cuentaRepository->findByPers(null,$user->getEmpresaActual());
                break;
            case 7://tramitador
                $query=$cuotaRepository->findVencimiento($user->getId(),null,null,$filtro,7,false,$fecha,$conrestriccion,false,null,$rut);
                $companias=$cuentaRepository->findByPers(null,$user->getEmpresaActual());
                break;
            case 6: //abogado
                $query=$cuotaRepository->findVencimiento($user->getId(),null,null,$filtro,6,false,$fecha,$conrestriccion,false,null,$rut);
                $companias=$cuentaRepository->findByPers(null,$user->getEmpresaActual());
                break;
            case 11://Administrativo
                //$query=$contratoRepository->findByPers(null,$user->getEmpresaActual(),$compania,$filtro,null,$fecha,true);
                $query=$cuotaRepository->findVencimiento(null,null,null,$filtro,null,false,$fecha,$conrestriccion,false,null,$rut);
                $companias=$cuentaRepository->findByPers(null,$user->getEmpresaActual());
            ...
            default:
                //$query=$contratoRepository->findByPers(null,null,$compania,$filtro,null,$fecha,true);
                $query=$cuotaRepository->findVencimiento($user->getId(),null,null,$filtro,null,false,$fecha,$conrestriccion,false,null,$rut);
                $companias=$cuentaRepository->findByPers(null);
     ...
     */
    public function resumen(ContratoRepository $contratoRepository, 
                            CuotaRepository $cuotaRepository,
                            PagoRepository $pagoRepository,
                            PaginatorInterface $paginator,
                            ModuloPerRepository $moduloPerRepository,
                            Request $request,
                            CuentaRepository $cuentaRepository,
                            ColaboradorRepository $colaboradorRepository): Response
    {
        ...
        if(null !== $request->query->get('bColaborador') && $request->query->get('bColaborador')!=""){
            $bUsuarioRegistro=$request->query->get('bColaborador');
        }
        if(null !== $request->query->get('bFolio') && $request->query->get('bFolio')!=''){
            $folio=$request->query->get('bFolio');
            $otros=" (co.folio= '$folio' or co.agenda = '$folio') ";
        ...
        switch($user->getUsuarioTipo()->getId()){
            ...
            case 12:
                $query=$pagoRepository->findByPers(null,null,$compania,$filtro,$fecha,$bUsuarioRegistro);
                $total=$pagoRepository->findByPersCount(null,null,$compania,$filtro,$fecha,$bUsuarioRegistro);
                $companias=$cuentaRepository->findByPers(null,$user->getEmpresaActual());

                break;
            case 11://Administrativo
                $query=$pagoRepository->findByPers($user->getId(),null,$compania,$filtro,$fecha,$bUsuarioRegistro);
                $total=$pagoRepository->findByPersCount($user->getId(),null,$compania,$filtro,$fecha,$bUsuarioRegistro);
                $companias=$cuentaRepository->findByPers(null,$user->getEmpresaActual());
             break;
        ...
        return $this->render('pago/resumen.html.twig', [
            ...
            'bUsuarioRegistro'=>$bUsuarioRegistro,
            'bColaborador'=>$bUsuarioRegistro,
     ...
     */
    public function verpagosShow(Request $request, 
                                Contrato $contrato,
                                PagoRepository $pagoRepository,
                                ModuloPerRepository $moduloPerRepository,
     ...
     */
    public function detallepagos(Request $request, Cuota $cuota): Response
    {
                        ...
                        ModuloPerRepository $moduloPerRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            try{
                ...
                $errorSistema->setMensaje("Se ha registrado un nuevo pago para el contrato con folio: ".$contrato->getFolio());
                ...
                $errorSistema = new ErrorSistemaLog();            
                $errorSistema->setMensaje("Se asocia pago a cuota en contrato con folio: ".$contrato->getFolio());
                $errorSistema->setFecha(new \DateTime(date("Y-m-d H:i:s")));
                $errorSistema->setModulo("pago_new_".$pago->getId());
                $entityManager->persist($errorSistema);
                $entityManager->flush();
                ...
                $errorSistema->setFecha(new \DateTime(date("Y-m-d H:i:s")));
                $errorSistema->setModulo("pago_new");
                $entityManager->persist($errorSistema);
                $entityManager->flush();
            }
    ...
    public function existeComprobante(Contrato $contrato,Request $request,PagoRepository $pagoRepository): Response{
        ...
        $pago=$pagoRepository->findOneBy(['ncomprobante'=>$comprobante,'contrato'=>$contrato->getId(),'anulado'=>null]);
       // echo $pago->getId();
        if($pago!=null){
                        ...
                        PagoRepository $pagoRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $fechaPago=$request->request->get('fechaPago');
            $pagoTotal = $pagoRepository->findByTotalPorContrato($contrato->getId());
            if($pagoTotal['total']>=$pago->getMonto()){
                ...
                $pagoCuotasRepository->asociarNC($contrato,$pago);
                $primeraCuotaVigente=$cuotaRepository->findOneByPrimeraVigenteAPagar($contrato->getId());
                if($primeraCuotaVigente != null ){
                    $contrato->setProximoVencimiento($primeraCuotaVigente->getFechaPago());
                    $entityManager->persist($contrato);
                    $entityManager->flush();
                ...
                exec("php {$projectDir}/bin/console actualizar-contrato {$contrato->getId()} > /dev/null 2>&1 &");
                
                return $this->redirectToRoute('verpagos_index',['id'=>$contrato->getId()]); 
            }else{
                $error_toast='<div class="alert alert-danger" role="alert">El monto de la nota de crédito no puede ser mayor al total pagado. Total pagado: $'.number_format($pagoTotal['total'],0,",",".").'</div>';
            }                
    ...
    /**
     * @Route("/{id}/edit_boleta", name="pago_edit_boleta", methods={"GET","POST"})
     */
    public function editBoleta(Request $request, Pago $pago,ModuloPerRepository $moduloPerRepository): Response
    {
        $this->denyAccessUnlessGranted('edit','pago');
        $user=$this->getUser();
        $pagina=$moduloPerRepository->findOneByName('pago',$user->getEmpresaActual());
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('pago_resumen');
           
        }

        return $this->render('pago/edit_boleta.html.twig', [
            'pago' => $pago,
            'contrato'=>$contrato,
            'form' => $form->createView(),
            'pagina'=>'Editar N° boleta '.$pagina->getNombre(),
            'etapa'=>2,
        ]);
    }
    /**
