...
class NocontrataController extends AbstractController
{
    ...
    public function index(AgendaRepository $agendaRepository,CuentaRepository $cuentaRepository,PaginatorInterface $paginator,ModuloPerRepository $moduloPerRepository,Request $request): Response
    {
        ...
        if(null !== $request->query->get('bAgendaId') && $request->query->get('bAgendaId')!=''){
            $folio=$request->query->get('bAgendaId');
            $fecha=" a.id = ".$folio;
        ...
        switch($user->getUsuarioTipo()->getId()){
            ...
            case 5:
                ...
                $query=$agendaRepository->findByPers($user->getId(),null,$compania,'8',$filtro,1,$fecha);
                $companias=$cuentaRepository->findByPers($user->getId());
                break;
            default:
                $query=$agendaRepository->findByPers($user->getId(),null,$compania,'8',$filtro,3,$fecha);
    ...
    /**
     * @Route("/reasignar", name="nocontrata_reasignar", methods={"GET","POST"})
     */
    public function reasignar(Request $request,UsuarioRepository $usuarioRepository,AgendaRepository $agendaRepository):Response
    {
        ...
        return $this->render('nocontrata/reasignar.html.twig', [
