...
class PjudMovimientosController extends AbstractController
{
                                ...
                                PjudNotificacionesRepository $pjudNotificacionesRepository): JsonResponse
    {
        ...
        $pjudMovimientos = $pjudMovimientoRepository->findBy(['pjudCausa'=>$pjudCausa->getId(),'cuadernoId'=>$cuadernoId],['indice'=>'Asc']);
        try{
    ...
    function actualizarCausa(Causa $causa, PjudScraping $pjudScraping): JsonResponse
    {
        ...
        try{
            ...
            $causa->setEstadoConsultaPjud("Error");
            $entityManager->persist($causa);    
            $entityManager->flush();
                
            return $this->json(["error"=>"En estos momentos no podemos atender tu consulta, intentalo mas tarde"], 400);
        }
