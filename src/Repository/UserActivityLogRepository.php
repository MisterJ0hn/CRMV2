...
class UserActivityLogRepository extends ServiceEntityRepository
{
    ...
    public function findByFiltros(?int $usuarioId, ?string $desde, ?string $hasta, ?string $modulo, ?int $perfilId = null, ?string $accion = null): \Doctrine\ORM\Query
    {
        ...
        if ($modulo) {
            $qb->andWhere('l.controlador LIKE :modulo')
               ->setParameter('modulo', '%' . $modulo . 'Controller%');
        }

        if ($accion) {
            $qb->andWhere('l.controlador LIKE :accion')
    ...
    /**
     * Devuelve las acciones (métodos) distintas registradas para un módulo dado.
     * Extrae la parte después de '::' del campo controlador.
     * Ej: "App\Controller\ContratoController::cartera" → "cartera"
     */
    public function getAccionesDistintos(string $modulo): array
    {
        $rows = $this->createQueryBuilder('l')
            ->select('DISTINCT l.controlador')
            ->where('l.controlador IS NOT NULL')
            ->andWhere('l.controlador LIKE :modulo')
            ->setParameter('modulo', '%' . $modulo . 'Controller%')
            ->orderBy('l.controlador', 'ASC')
            ->getQuery()
            ->getScalarResult();
        ...
        foreach ($rows as $row) {
            $ctrl = $row['controlador'];
            // Extraer solo el nombre del método después de '::'
            if (preg_match('#::(\w+)$#', $ctrl, $m)) {
                $metodo = $m[1];
                if (!in_array($metodo, $acciones)) {
                    $acciones[] = $metodo;
                }
            }
        }
