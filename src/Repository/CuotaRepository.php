...
class CuotaRepository extends ServiceEntityRepository
{
    ...
    public function findOneByPrimeraVigenteAPagar($contrato,$isMulta=false): ?Cuota
    {
   ...
   /* public function totalesUsuario()
    ...
    public function findCuotaPagoAutomaticoPendiente($contratoId=null){
        $query=$this->createQueryBuilder('c')
        ->join('c.contrato','co')
        ->where('c.monto>c.pagado or c.pagado is null')
        ->andWhere('c.anular is null or c.anular = false')
        ->andWhere("co.EstadoSuscripcion = 'ACTIVA'")
        ->andWhere("c.fechaPago <='".date('Y-m-d H:i:s')."'");
        if($contratoId){
            $query->andWhere('c.contrato = '.$contratoId);
        }
        return $query->getQuery()
            ->getResult();
    }
