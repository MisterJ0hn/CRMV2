        ...
        */
        ...
        if($conrestriccion==true){
            if($vigente){
                ...
                $query->andWhere('
                    c.numero = (
                        SELECT MIN(c2.numero)
                        FROM App\Entity\Cuota c2
                        WHERE c2.contrato = co
                        AND (c2.monto > (c2.pagado + '.$configuracion->getDeudaMinima().') OR c2.pagado IS NULL)
                        AND (c2.anular IS NULL OR c2.anular = false)
                    )
                ');
            }else{
