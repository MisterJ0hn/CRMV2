...
class LineaTiempoEtapasRepository extends ServiceEntityRepository
{
    ...
    public function obtenerEtapaPendiente(int $causaId,int $lineaTiempoId){
        $query = $this->createQueryBuilder('l')
        ->andWhere("l.lineaTiempo=".$lineaTiempoId);

        $query->andWhere(" not exists (select 1 from App\Entity\LineaTiempoTerminada lt where lt.lineaTiempoEtapas = l and lt.causa = ".$causaId.")");
