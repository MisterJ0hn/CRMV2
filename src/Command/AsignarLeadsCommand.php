        ...
        foreach($agendas as $agenda){
            ...
            foreach($agendadores as $agendador){
                ...
                if(intval($leads_asignados[0]['leads_asignados'])<$leads){
                    ...
                    //10-06-26: desconecto movatec para evitar errores por token, ya que el login se esta haciendo en otro proceso, por lo que el token no se comparte entre procesos, por lo que se debe hacer login en este proceso para obtener el token, pero como el login tiene un log asociado, esto genera muchos logs de error en movatec, por lo que se decide desconectar movatec de este proceso y solo asignar los leads en el crm, para luego hacer un proceso aparte que envie los leads a movatec
                    ...
                    shell_exec("cd ". $this->container->getParameter('url_raiz').";php bin/console app:movatec-crea-lead  --agendaId ".$agenda->getId()." > /dev/null 2>&1 &");
                    
                    //iniciamos conversación con adereso async.         
                    shell_exec("cd ". $this->container->getParameter('url_raiz').";php bin/console adereso:iniciar-conversacion  --agendaId ".$agenda->getId()." > /dev/null 2>&1 &");
